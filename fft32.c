...
__attribute__((always_inline)) void fft16(int* array) {
    int tar, tai, lar, lai, re, im;

    // top butterfly
    for(int i = 0; i < 8; i++)
    {
        tar = array[i] + array[i+16];
        tai = array[i+8] + array[i+24];
        array[i] = tar;
        array[i+8] = tai;
    }
    fft8(array);
    ...
    for(int i = 0; i < 8; i++)
    {
        lar = array[i] - array[i+16];
        lai = array[i+8] - array[i+24];
        array[i+16] = lar;
        array[i+24] = lai;
    }
    for(int i = 1; i < 8; i++)
    {
        int idx = i + 16;
        re = array[idx] * cos16[i] - array[idx+8] * sin16[i];
        im = array[idx] * sin16[i] + array[idx+8] * cos16[i];
        array[idx] = re >> NORM;
        array[idx+8] = im >> NORM;
    }
    fft8(array+16);
...
void fft32()
{
    ...
    for(int i = 0; i < 16; i++)
    {
        tar = goutput[i] + goutput[i+32];
        tai = goutput[i+16] + goutput[i+48];
        goutput[i] = tar;
        goutput[i+16] = tai;
    }
    */
    ...
    {
        int idx = i + 32; 
        lar = goutput[i] - goutput[i+32];
        lai = goutput[i+16] - goutput[i+48];


        goutput[i+32] = lar;//(lar + cos32[i]);
        goutput[i+48] = lai;//lai>>NORM;
/*
        re = lar * cos32[i] - lai * sin32[i];
        im = lar * sin32[i] + lai * cos32[i];

        goutput[idx] = re >> NORM;
        goutput[idx+16] = im >> NORM;
        */
    }
    fft16(goutput+32);
}
